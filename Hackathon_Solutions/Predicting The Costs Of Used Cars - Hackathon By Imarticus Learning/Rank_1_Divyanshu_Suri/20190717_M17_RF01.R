...
combined04[is.na(combined04$New_Price),"New_Price"] <- 0
combined04[is.na(combined04$Lag_Price2),"Lag_Price2"] <- 0
combined04[is.na(combined04$Lag_Price4),"Lag_Price4"] <- 0
...
for (i in 1:num_folds)
{
          ...
          RF_mod_1 <- randomForest( x = trainData[,feature.names],
                                    ...
                                    mtry = 25,
                                    ntree = 100,
                                    sampsize = round(0.6*nrow(trainData)),
                                    nodesize = 5,
                                    nPerm = 5,
                                    do.trace = TRUE,
                                    keep.forest = TRUE,
...
write_csv(test02, "C:\\Kaggle\\Cars\\Test_Scored\\20190717_RF01_TEST_DS.csv")
write_csv(cv_testData, "C:\\Kaggle\\Cars\\CV_Scored\\20190717_RF01_CVTRAIN_DS.csv")

sub <- data.frame(Price = test02$pred_XGB_Final)
write.xlsx(sub, "C:\\Kaggle\\Cars\\Submission\\20190717_RF01_TEST_DS.xlsx", colNames = TRUE, rowNames = FALSE)
