...
names(combined01)
feature.names <- names(combined01)[!(names(combined01) %in% c("id","Price","New_Price","Name","CarCompName"))]
...
tr1 <- read.csv("C:\\Kaggle\\Cars\\CV_Scored\\20190715_XGB05_CVTRAIN_DS.csv")
tr2 <- read.csv("C:\\Kaggle\\Cars\\CV_Scored\\20190715_XGB06_CVTRAIN_DS.csv")
tr3 <- read.csv("C:\\Kaggle\\Cars\\CV_Scored\\20190715_XGB07_CVTRAIN_DS.csv")
tr4 <- read.csv("C:\\Kaggle\\Cars\\CV_Scored\\20190715_XGB08_CVTRAIN_DS.csv")
tr5 <- read.csv("C:\\Kaggle\\Cars\\CV_Scored\\20190715_XGB09_CVTRAIN_DS.csv")
tr6 <- read.csv("C:\\Kaggle\\Cars\\CV_Scored\\20190716_XGB10_CVTRAIN_DS.csv")

ts1 <- read.csv("C:\\Kaggle\\Cars\\Test_Scored\\20190715_XGB05_TEST_DS.csv")
ts2 <- read.csv("C:\\Kaggle\\Cars\\Test_Scored\\20190715_XGB06_TEST_DS.csv")
ts3 <- read.csv("C:\\Kaggle\\Cars\\Test_Scored\\20190715_XGB07_TEST_DS.csv")
ts4 <- read.csv("C:\\Kaggle\\Cars\\Test_Scored\\20190715_XGB08_TEST_DS.csv")
ts5 <- read.csv("C:\\Kaggle\\Cars\\Test_Scored\\20190715_XGB09_TEST_DS.csv")
ts6 <- read.csv("C:\\Kaggle\\Cars\\Test_Scored\\20190716_XGB10_TEST_DS.csv")

train02 <- sqldf("select a.*,
                         b.Predicted_Scores as Pred01,
                         c.Predicted_Scores as Pred02,
                         d.Predicted_Scores as Pred03,
                         e.Predicted_Scores as Pred04,
                         f.Predicted_Scores as Pred05,
                         g.Predicted_Scores as Pred06
                  from train02 as a inner join tr1 as b on a.id = b.id
                                    inner join tr2 as c on a.id = c.id
                                    inner join tr3 as d on a.id = d.id
                                    inner join tr4 as e on a.id = e.id
                                    inner join tr5 as f on a.id = f.id
                                    inner join tr6 as g on a.id = g.id
                  order by a.id")

test02 <- sqldf("select a.*,
                        log(b.pred_XGB_Final+1) as Pred01,
                        log(c.pred_XGB_Final+1) as Pred02,
                        log(d.pred_XGB_Final+1) as Pred03,
                        log(e.pred_XGB_Final+1) as Pred04,
                        log(f.pred_XGB_Final+1) as Pred05,
                        log(g.pred_XGB_Final+1) as Pred06
                 from test02 as a inner join ts1 as b on a.id = b.id
                                   inner join ts2 as c on a.id = c.id
                                   inner join ts3 as d on a.id = d.id
                                   inner join ts4 as e on a.id = e.id
                                   inner join ts5 as f on a.id = f.id
                                   inner join ts6 as g on a.id = g.id
                 order by a.id")

feature.names <- c(feature.names,"Pred01","Pred02","Pred03","Pred04","Pred05","Pred06")
feature.names
...
for (i in 1:num_folds)
{
  ...
  XGB_mod_1 <- xgb.train(   params = param,
                            ...
                            print.every.n = 25
  )#[101]	train-rmse:0.101873	val-rmse:0.127776
...
write_csv(test02, "C:\\Kaggle\\Cars\\Test_Scored\\20190717_XGB11_TEST_DS.csv")
write_csv(cv_testData, "C:\\Kaggle\\Cars\\CV_Scored\\20190717_XGB11_CVTRAIN_DS.csv")

sub <- data.frame(Price = test02$pred_XGB_Final)
write.xlsx(sub, "C:\\Kaggle\\Cars\\Submission\\20190717_XGB11_TEST_DS.xlsx", colNames = TRUE, rowNames = FALSE)
