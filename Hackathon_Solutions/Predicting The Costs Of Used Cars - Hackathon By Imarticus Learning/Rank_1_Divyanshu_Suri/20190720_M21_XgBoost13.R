...
train02 <- train02[train02$Name %in% test02$Name,]
...
for (i in 1:num_folds)
{
  ...
  XGB_mod_1 <- xgb.train(   params = param,
                            ...
                            print.every.n = 25
  )#[1367]	train-rmse:0.042899	val-rmse:0.119526
...
tst01 <- read.xlsx("C:\\Kaggle\\Cars\\Submission\\20190718_XGB12_TEST_DS.xlsx",
                   sheet = 1,
                   startRow = 1,
                   colNames = TRUE)

test02$pred_XGB_Final2 <- test02$pred_XGB_Final
test02$pred_XGB_Final2 <- ifelse(!(test01$Name %in% unique(train01$Name)),tst01$Price,test02$pred_XGB_Final2)
summary(test02$pred_XGB_Final2)

write_csv(test02, "C:\\Kaggle\\Cars\\Test_Scored\\20190720_XGB13_TEST_DS.csv")
write_csv(cv_testData, "C:\\Kaggle\\Cars\\CV_Scored\\20190720_XGB13_CVTRAIN_DS.csv")

sub <- data.frame(Price = test02$pred_XGB_Final2)
write.xlsx(sub, "C:\\Kaggle\\Cars\\Submission\\20190720_XGB13_TEST_DS.xlsx", colNames = TRUE, rowNames = FALSE)
