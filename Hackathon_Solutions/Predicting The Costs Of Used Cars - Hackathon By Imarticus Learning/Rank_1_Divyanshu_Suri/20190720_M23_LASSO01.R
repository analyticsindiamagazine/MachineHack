...
for(fname in feature.names) {
  if(sum(is.na(combined06[,fname])) > 0) {
    combined06[,paste(fname,"_M",sep="")] <- ifelse(is.na(combined06[,fname]),1,0)
    combined06[is.na(combined06[,fname]),fname] <- mean(combined06[,fname],na.rm = TRUE)
    feature.names <- c(feature.names,paste(fname,"_M",sep=""))
  }
}
...
lambda_vals <- c(0.00035)
best_rmse <- 9999999999999

for(s in lambda_vals)
{
  for (i in 1:num_folds)
  {
    cat("RUNNING FOR ",i," ")
    ...
    LASSO_mod_1 <- glmnet(x = as.matrix(trainData[,feature.names]),
                          y = trainData[,"Price"],
                          family = "gaussian",
                          lambda = s,
                          alpha = 1)
    
    preds <- (predict(LASSO_mod_1,as.matrix(testData[,feature.names])))
    
    test02[,paste("pred_LASSO_",i,sep="")] <- (predict(LASSO_mod_1,as.matrix(test02[,feature.names])))
    ...
    if (i == 1)
    {
      ...
      preds_sub <- test02[,paste("pred_LASSO_",i,sep="")]
    }
    else
    {
      cv_preds <- c(cv_preds,preds)
      cv_acts <- c(cv_acts,(testData$Price))
      cv_ids <- c(cv_ids,testData$Id)
      
      preds_sub <- preds_sub + test02[,paste("pred_LASSO_",i,sep="")]
    }
  }
  if(rmse((cv_preds),(cv_acts)) < best_rmse)
  {
    cat("\nFINAL RMSE : ",rmse((cv_preds),(cv_acts))," ; lambda : ",s,"\n")
    best_rmse <- rmse((cv_preds),(cv_acts))
  }
}

#FINAL RMSE :  0.07903756  ; lambda :  0.00035
#LB SCORE : 0.9292

preds_sub <- preds_sub / num_folds
summary(preds_sub)
test02$pred_Final <- (10**preds_sub)-1
summary(test02$pred_Final)

write_csv(test02, "C:\\Kaggle\\Cars\\Test_Scored\\20190721_LASSO01_TEST_DS.csv")
write_csv(cv_testData, "C:\\Kaggle\\Cars\\CV_Scored\\20190721_LASSO01_CVTRAIN_DS.csv")

sub <- data.frame(Price = test02$pred_Final)
write.xlsx(sub, "C:\\Kaggle\\Cars\\Submission\\20190721_LASSO01_TEST_DS.xlsx", colNames = TRUE, rowNames = FALSE)
